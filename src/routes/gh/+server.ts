...
const parseJson = z.string().min(1).transform(s => JSON.parse(s));
...
async function handleWebhookRequest(request: WebhookRequest) {
    if (!request.issue || request.action != "opened" || request.repository?.full_name != "25huizengek1/ViTune") return;
    try {
        let response = await fetch(`https://api.github.com/repos/${request.repository.full_name}/issues/${request.issue.number}`, {
            method: "PATCH",
            headers: {
        ...
        response = await fetch(`https://api.github.com/repos/${request.repository.full_name}/issues/${request.issue.number}/comments`, {
            method: "POST",
            headers: {
                Accept: "application/vnd.github+json",
                Authorization: `Bearer ${GH_API_KEY}`,
            ...
            body: JSON.stringify({
                body: `
This issue has been automatically closed do to ViTune not being maintained anymore.
...
function verifySignature(payload: string, webhookSecret: string, webhookSignature: string): boolean {
...
export const POST: RequestHandler = async ({ request }) => {
    ...
    if (!subscribedEvents.includes(eventType))
        error(400, "not subscribed to such event");
